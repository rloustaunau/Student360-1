...
@Component({
  selector: 'app-ytd-grade-level',
  templateUrl: './ytd-grade-level.component.html',
  styleUrls: ['./ytd-grade-level.component.css']
})

export class YtdGradeLevelComponent {
  data: YtdRegionRate[];
  loading = false;
  ...
  ngOnInit() {
    this.loading = true;
    this.apiService.report.getYtdGradeLevels().subscribe(result => {
      this.data = result;
  ...
  generateChartData() {
    ...
    this.chartLabels = [];
    this.generateColors();
    this.chartLabels = this.data.map(x => x.gradeLevel + ' ');
    var dataObject = {
