...
@Component({
  selector: 'app-ytd-level',
  templateUrl: './ytd-level.component.html',
  styleUrls: ['./ytd-level.component.css']
})

export class YtdLevelComponent {
  data: YtdRegionRate[];
  loading = false;
  ...
  ngOnInit() {
    this.loading = true;
    this.apiService.report.getYtdSchoolLevels().subscribe(result => {
      this.data = result;
  ...
  generateChartData() {
    ...
    this.chartLabels = [];
    this.generateColors();
    this.chartLabels = this.data.map(x => x.schoolLevel + ' ');
    var dataObject = {
